...
namespace MphRead.Mods.MapGen
{
    ...
    public sealed class MapCatalog
    {
        ...
        public MapCatalog(params string[] directories)
        {
            if (directories.Length == 0) throw new ArgumentException("Supply at least one map directory.", nameof(directories));
            _directories = directories.Select(p => Path.TrimEndingDirectorySeparator(Path.GetFullPath(p))).Distinct(OperatingSystem.IsWindows()
                ? StringComparer.OrdinalIgnoreCase : StringComparer.Ordinal).ToArray();
            DirectoryPath = _directories[0];
        }
        ...
        public IReadOnlyList<MapCatalogEntry> Refresh(bool preferPackages = true)
        {
            ...
            // Roots are ordered by priority: writable user maps before shipped maps.
            foreach (string directory in _directories)
            {
                if (!Directory.Exists(directory)) continue;
                string installed=Path.Combine(directory,".installed");
                var files = Directory.EnumerateFiles(directory, "*", options)
                    ...
                    .Where(p => !Path.GetRelativePath(directory, p).Split(Path.DirectorySeparatorChar, Path.AltDirectorySeparatorChar)
                ...
                foreach (string path in files)
                {
                    ...
                    try { definition = MapDefinition.Load(path); CustomRooms.PrepareImportForBuild(definition); validation = MapValidator.Validate(definition); }
                    ...
                    if (definition != null && validation.IsValid)
                    {
                        if (names.TryGetValue(definition.Name, out var prior))
                        {
                            ...
                            if (sameId && prior.Validation.IsValid && (roots[prior.Path] != directory
                                || MapBundle.Is(prior.Path) != MapBundle.Is(path)||Path.GetDirectoryName(prior.Path)==installed||Path.GetDirectoryName(path)==installed)) continue;
